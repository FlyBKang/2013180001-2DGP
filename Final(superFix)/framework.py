...
def UI_Draw():
    ...
    Texture.Font.draw(670,460, '%d' %(Global.g_Player.BulletPower+3), (255, 255, 255))
    Texture.Font.draw(670,415, '%d' %Global.g_Player.BulletNum, (255, 255, 255))
...
def MonsterDraw():
    ...
    if (Global.g_Boss.live == True):
        if((int)(Global.g_Level) == 0):
            Texture.P_Boss1.clip_composite_draw(0, 0, 79, 79, Global.g_Boss.Degree * angle, "", Global.g_Boss.X,
                                            Global.g_Boss.Y, Global.g_Boss.W, Global.g_Boss.H)
        elif((int)(Global.g_Level) == 1):
            Texture.P_Boss2.clip_composite_draw(0, 0, 74, 74, Global.g_Boss.Degree * angle, "", Global.g_Boss.X,
                                            Global.g_Boss.Y, Global.g_Boss.W, Global.g_Boss.H)
            for i in range(0,3):
                Global.BossFX[i].Set(Global.g_Boss.X+60*math.sin( Global.BossFX[i].Degree*3),Global.g_Boss.Y+60*math.cos( Global.BossFX[i].Degree*3),20,20)
                Texture.P_player_alpha.clip_composite_draw(65, 0, 65, 65, Global.BossFX[i].Degree * angle, "", Global.BossFX[i].X,
                                                           Global.BossFX[i].Y, Global.BossFX[i].W, Global.BossFX[i].H)
                Global.BossFX[i].Degree += Timer.Time_Frame
        else:
    ...
    for Arr in Global.g_MonsterBulletArr:
        if(Arr.Live == True):
            ...
            if(Arr.Type == 0):
                ...
                Texture.P_bullet.clip_draw(5 * 63, 0, 62, 64, Arr.X, Arr.Y, Arr.W, Arr.H)
            if(Arr.Type == 7):
                Texture.P_bullet.clip_draw(0 * 63, 0, 62, 64, Arr.X, Arr.Y, Arr.W, Arr.H)
            elif(Arr.Type == 8):
                Texture.P_bullet.clip_draw(5 * 63, 0, 62, 64, Arr.X, Arr.Y, Arr.W, Arr.H)
            if(Arr.Type == 9):
                Texture.P_bullet.clip_draw(0 * 63, 0, 62, 64, Arr.X, Arr.Y, Arr.W, Arr.H)
                ...
                Texture.P_bullet.clip_draw(4 * 63, 0, 62, 64, Arr.X, Arr.Y, Arr.W, Arr.H)
            elif (Arr.Type == 12):
                Texture.P_bullet.clip_draw(4 * 63, 0, 62, 64, Arr.X, Arr.Y, Arr.W, Arr.H)
